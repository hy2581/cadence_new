...
- Sequences: `fa_uvm_reg_sequence`, `fa_uvm_attention_job_sequence`, `fa_uvm_causal_e2e_sequence`.
...
- Environment/tests: `fa_uvm_env`, `fa_uvm_base_test`, `fa_uvm_causal_e2e_test`, and `fa_uvm_smoke_test`.
...
- Reset defaults: `CTRL=0`, `STATUS=0`, `CFG=0`, base registers `0`, `STRIDE_BYTES=128`, `NEG_LARGE=0xFFFF8000`, `SCALE=0x20`, counters `0`.
- RW registers: `CFG`, Q/K/V/O base low/high, `STRIDE_BYTES`, `NEG_LARGE`, `SCALE`.
- RO registers: `CYCLES`, `RD_BYTES`, `WR_BYTES` ignore writes.
...
- Control flow: soft reset, start, busy observed, done observed, done W1C clear, causal enabled.
- DMA regions: Q, K, V reads and O writes.
- AXI channels: AW, W, B, AR, R handshakes.
- Burst lengths: Q/O tile bursts and K/V tile bursts.
- Performance bucket: pass bucket under 300k cycles.
