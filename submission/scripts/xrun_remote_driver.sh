...
# ---- 2. Run full UVM test suite ----------------------------------
cd "$(dirname "$0")/.."
echo "[xrun-driver] cwd=$(pwd)"
bash scripts/xrun_uvm_all.sh 2>&1 | tee "$OUTDIR/xrun_all.log"
...
# Keep tarball reasonably small: logs + merged coverage only (no INCA_libs, no waves).
TMPPACK=$(mktemp -d)
